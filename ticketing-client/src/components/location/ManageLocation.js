...
// import logo from '../location/melody.png'; // Import your logo image

const generatePDF = (newLocation) => {
  const doc = new jsPDF();
  
  // Add logo at the top
  // doc.addImage(logo, 'PNG', 10, 10, 50, 30); // Adjust the position and size as needed

  // Add report header
  doc.setFontSize(16);
  doc.text('Melody Mesh', 70, 20); // Center the brand name
  doc.setFontSize(12);
  doc.text('Report generated by computer', 60, 30); // Statement below brand name

  // Add form data
  const { locationName, notes, contactPerson, contactPhone, contactEmail, capacity, parking, accessibleFeatures } = newLocation;
  const formData = [
  ...
  formData.forEach((line, index) => {
    doc.text(line, 10, 50 + (index * 10)); // Adjust position for each line
  });
          ...
          {filteredEvents.map((event) => (
            <Grid item xs={12} sm={6} key={event._id}>
              <Box border={1} padding={2} borderRadius={4} borderColor="grey.300" >
                ...
                <Button
                sx={{ marginLeft: 2 }}
                color="primary" variant="contained" onClick={() => generatePDF(newLocation)}>
                    Generate PDF
                  </Button>
              </Box>
            </Grid>
